...
public class SendMsgKafka {
	...
	private void handleParams(BaseMsg baseMsg, kafkaMsg kafkaMsg) {
		...
		kafkaMsg.setOpenSSL(baseMsg.isOpenSSL());
		kafkaMsg.setSslp(baseMsg.getSslp());
	}
	...
	public Properties kafkaController2(BaseMsg baseMsg) {
		...
		if (baseMsg.isOpenSSL()) {
			System.out.println(baseMsg.getSslp());

			// 添加ssl认证
			props.put("security.protocol", "SSL");

			props.put("ssl.truststore.location", baseMsg.getSslp().getTruststoreLocation());

			props.put("ssl.truststore.password", baseMsg.getSslp().getTruststorePwd());

			props.put("ssl.keystore.location", baseMsg.getSslp().getKeystoreLocation());

			props.put("ssl.keystore.password", baseMsg.getSslp().getKeystorePwd());

			props.put("ssl.key.password", baseMsg.getSslp().getKeyPwd());
		}
